...
public class AtomIteratorArrayListAdjacent implements AtomIteratorAtomDependent, java.io.Serializable {
     ...
     */
    public AtomIteratorArrayListAdjacent(Direction direction) {
    ...
    public int size() {
        int size = 0;
        if(direction != Direction.DOWN && (firstCursor < list.getAtomCount()-1)) {
            size++;
        }
        if(direction != Direction.UP && (firstCursor > 0)) {
            size++;
        }
        return size;
    }
     ...
     */
    public void reset() {
        upListNow = ((direction != Direction.DOWN) && (firstCursor < list.getAtomCount()-1));
        dnListNow = ((direction != Direction.UP) && (firstCursor > 0));
    }
