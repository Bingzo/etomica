...
public class VirialDiagramsPT {
    ...
    public static void main(String[] args) {
        ...
        VirialDiagramsPT virialDiagrams = new VirialDiagramsPT(n, multibody, flex, true);
    ...
    public VirialDiagramsPT(int n, boolean multibody, boolean flex) {
        this(n, multibody, flex, false);
    }

    public VirialDiagramsPT(int n, boolean multibody, boolean flex, boolean interactive) {
    ...
    public void setOrderBeta(int newOrder) {
        orderBeta = newOrder;
    }
    ...
    public Set<Graph> getMSMCGraphs(boolean connectedOnly, int order) {
        if (p == null) {
            makeVirialDiagrams();
        }
        ...
        for (Graph g : allP) {
            ...
            if (fieldCount == n) {
        		int gOrder = 0;
        		for (Edge e : g.edges()) {
        			if (e.getColor() == 'r') continue;
        			gOrder += (e.getColor()-aBonds[0])+1;
        			if (gOrder>aBonds.length) break;
        		}
        		if  (order == gOrder) {
        			pn.add(g);
        		}
    ...
    public ClusterSum makeVirialCluster(MayerFunction r, MayerFunction[] a) {
        if (p == null) {
            makeVirialDiagrams();
        }
        Set<Graph> pn = getMSMCGraphs(false, orderBeta);
//        ClusterViewer.createView("B51", pn);
        return makeVirialCluster(pn, r, a);
    }
    
    public ClusterSum makeVirialCluster(Set<Graph> graphs, MayerFunction r, MayerFunction[] a) {
        ...
        for (Graph g : graphs) {
            ...
            int nDiagrams = populateBonds(g, allBonds, weights, false);
            if (nDiagrams > 0 && flex) {
                populateBonds(g, allBonds, weights, true);
            }
        }
        ...
        MayerFunction[] allf = new MayerFunction[a.length+1];
        allf[0] = r;
        System.arraycopy(a, 0, allf, 1, a.length);
        return new ClusterSum(allBonds.toArray(new ClusterBonds[0]), w, allf);
    }
    ...
    public int populateBonds(Graph g, List<ClusterBonds> allBonds, List<Double> weights, boolean swap) {
        int rv = 0;
        ArrayList<int[]> rbonds = new ArrayList<int[]>();
        List<int[]>[] abondLists = new List[aBonds.length];
        for (int i=0; i<aBonds.length; i++) {
        	abondLists[i] = new ArrayList<int[]>();
        }
        rv = 1;
        if (allPermutations) {
                ...
                for (List<Byte> comp : multiBiComponents) {
                    ...
                    for (int id1 = 0; id1<comp.size()-1; id1++) {
                        for (int id2 = id1+1; id2<comp.size(); id2++) {
                            ...
                            if (edgeColor == rBond) {
                                rbonds.add(new int[]{n1,n2});
                            }
                            else {
                                boolean success = false;
                                for (int i=0; i<aBonds.length; i++) {
                                    if (edgeColor == aBonds[i]) {
                                        abondLists[i].add(new int[]{n1,n2});
                                        success = true;
                                        break;
                                    }
                                }
            ...
            int[][][] allBondsArrays = new int[1+aBonds.length][0][0];
            allBondsArrays[0] = rbonds.toArray(new int[0][0]);
            for (int i=0; i<aBonds.length; i++) {
            	allBondsArrays[i+1] = abondLists[i].toArray(new int[0][0]);
            }
            allBonds.add(new ClusterBonds(flex ? n+1 : n, allBondsArrays));
    ...
    public ClusterSumShell[] makeSingleVirialClusters(ClusterSum coreCluster, MayerFunction r, MayerFunction[] a, int order) {
        if (p == null) {
            makeVirialDiagrams();
        }
        ArrayList<ClusterSumShell> allClusters = new ArrayList<ClusterSumShell>();
        Set<Graph> pn = getMSMCGraphs(true, order);
        for (Graph g : pn) {
            List<Double> weights = new ArrayList<Double>();
            ArrayList<ClusterBonds> allBonds = new ArrayList<ClusterBonds>();
            populateBonds(g, allBonds, weights, false);
            if (flex) {
                populateBonds(g, allBonds, weights, true);
            }
            if (flex && cancelMap.get(g) != null) {
                Graph cg = cancelMap.get(g);
                populateBonds(cg, allBonds, weights, false);
                populateBonds(cg, allBonds, weights, true);
            }
            ...
            allf[0] = r;
            System.arraycopy(a, 0, allf, 1, a.length);
            allClusters.add(new ClusterSumShell(coreCluster, allBonds.toArray(new ClusterBonds[0]), thisW, allf));
        }
        return allClusters.toArray(new ClusterSumShell[0]);
    }

    public Set<Graph> getExtraDisconnectedVirialGraphs(int order) {
        if (p == null) {
            makeVirialDiagrams();
        }
    ...
    public void makeRhoDiagrams() {
        ...
        if (doShortcut && !multibody && !doKeepEBonds) {
            ...
            for (byte i=1; i<n+1; i++) {
                ...
                g.coefficient().setDenominator((int) SpecialFunctions.factorial(i));
                eXi.add(g);
...
outer:                      for (int i=1; i<biComp.size(); i++) {
        ...
        if (orderBeta == 0) {
            ...
            for (int i=0; !doExp && i<aBonds.length-1; i++) {
                ...
                splitParameters = new SplitParameters(Xbond, aBonds[i], Ybond, discardCriteria);
                newP = splitter.apply(newP, splitParameters);
            	Xbond++;
            }
        }
        p = makeGraphList();
...
//            ComponentSubstParameters[] csp = new ComponentSubstParameters[n+1];
        ...
        if (isInteractive) {
            Metadata.COLOR_MAP.put(rBond, "black");
            Metadata.COLOR_MAP.put('a', "orange");
            Metadata.COLOR_MAP.put('b', "blue");
            Metadata.COLOR_MAP.put('c', "red");
            topSet.clear();
            topSet.addAll(p);
            ...
            ClusterViewer.createView("P PVC", topSet);
        }
    ...
    public static class DiscardCriteria implements Property {
        private final int ii, orderBeta;
        private final char xX, rBond;

        /**
         * @param xX is the pseudo-bond, which represents a sum of high-order bonds
         * @param ii is the assumed order of an xX bond 
         * @param rBond is the reference bond
         * @param orderBeta the desired truncation order of the series
         */
        public DiscardCriteria(char xX, int ii, char rBond, int orderBeta) {
            this.ii = ii;
            this.xX = xX;
            this.rBond = rBond;
            this.orderBeta = orderBeta;
        }

        public boolean check(Graph graph) {
            // returns true if the graph should be discarded because it is too high in order
            int o = 0;
            for (Edge e : graph.edges()) {
                if (e.getColor() == rBond) continue;
                if (e.getColor() == xX) {
                    o += ii;
                }
                else {
