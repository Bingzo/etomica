 ...
 * 
 * @author shu
 * Date: April 26 2012, updated a little bit on 07/16/2012
 */
public class MayerIonicDiagram4qbonds {
    ...
    protected char fBond, eBond, mBond, kBond,   GBond, G1Bond, G2pBond, G3pBond,   G2Bond, G3Bond,G4Bond;
    protected char g1Bond, g2Bond,g3Bond,g4Bond,kg1Bond, kg2Bond, kg3Bond,kg4Bond;
    protected char q1Bond,q2Bond,q3Bond,q4Bond;//modified q bonds
    protected char kg1BondMinus, kg2BondMinus, kg3BondMinus,kg4BondMinus;
    // Let's begin!
    public static void main(String[] args) {
    	GraphImpl.useReverseEdges = true;
    	MetadataImpl.rootPointsSpecial = true;
        final int n = 2;// number of pts
        if (n>3){
        	throw new RuntimeException("It is beyond my inteligence! I am sorry!");
        }
        boolean multibody = false;
        boolean bondDecomp = true;
        boolean flex = false;
        MayerIonicDiagram4qbonds virialDiagrams = new MayerIonicDiagram4qbonds(n, multibody, flex, bondDecomp, true);
        virialDiagrams.setDoReeHoover(false);
        virialDiagrams.setDoShortcut(false);
        virialDiagrams.makeVirialDiagrams();
    }
    // constructor
    public MayerIonicDiagram4qbonds(int n, boolean multibody, boolean flex, int numSite, boolean bondDecomp) {
    	this(n, multibody, flex, bondDecomp, false);
    }
    // the other constructor
    public MayerIonicDiagram4qbonds(int n, boolean multibody, boolean flex, boolean bondDecomp, boolean interactive) {
        ...
        for (Node node1 : g.nodes()) {
            for (Node node2 : g.nodes()) {
                ...
                if (g.hasEdge(node1.getId(), node2.getId())) {
                    ...
                    else if (edgeColor == G3Bond) {
                    	gBonds.add(new int[]{n1,n2});
                    }
                    else if (edgeColor == G4Bond) {
                    	gBonds.add(new int[]{n1,n2});
                    }  
                    else if (edgeColor == G2pBond) {
                    	gBonds.add(new int[]{n1,n2});
                    }  
                    else if (edgeColor == G3pBond) {
                    	gBonds.add(new int[]{n1,n2});
                    } 
                    else if (edgeColor == g1Bond) {
                    	gBonds.add(new int[]{n1,n2});
                    ...
                    else if (edgeColor == kg3Bond) {
                    	gBonds.add(new int[]{n1,n2});
                    } 
                    else if (edgeColor == kg4Bond) {
                    	gBonds.add(new int[]{n1,n2});
                    } 
                    else if (edgeColor == kg1BondMinus) {
                    	gBonds.add(new int[]{n1,n2});
                    } 
                    else if (edgeColor == kg2BondMinus) {
                    	gBonds.add(new int[]{n1,n2});
                    } 
                    else if (edgeColor == kg3BondMinus) {
                    	gBonds.add(new int[]{n1,n2});
                    } 
                    else if (edgeColor == kg4BondMinus) {
                    	gBonds.add(new int[]{n1,n2});
                    } else {
    ...
    public void makeRhoDiagrams() {
        ...
        if (doShortcut) {
            ...
            colorOrderMap.put(G2pBond, 8);
            colorOrderMap.put(G3Bond, 9);
            colorOrderMap.put(g1Bond, 10);
            colorOrderMap.put(g2Bond, 11);
            colorOrderMap.put(g3Bond, 12);
            colorOrderMap.put(kg1Bond, 13);
            colorOrderMap.put(kg2Bond, 14);
            colorOrderMap.put(kg3Bond, 15);
            colorOrderMap.put(q1Bond, 16);
            colorOrderMap.put(q2Bond, 17);
            colorOrderMap.put(q3Bond, 18);
            colorOrderMap.put(kg1BondMinus, 19);
            colorOrderMap.put(kg2BondMinus, 20);
            colorOrderMap.put(kg3BondMinus, 21);
            colorOrderMap.put(G3pBond, 22);
            // add 4th order qbonds
            colorOrderMap.put(G4Bond, 23);
            colorOrderMap.put(g4Bond, 24);
            colorOrderMap.put(kg4Bond, 25);
            colorOrderMap.put(q4Bond, 26);
            colorOrderMap.put(kg4BondMinus, 27);
            ...
            System.out.println(n+"pts diagram, bond splitting up to q4-bond");
        ...
        Metadata.COLOR_MAP.put(GBond, "black");
        Metadata.COLOR_MAP.put(G1Bond, "black");
        Metadata.COLOR_MAP.put(G2Bond, "black");
        Metadata.COLOR_MAP.put(G3Bond, "black");
        Metadata.COLOR_MAP.put(G4Bond, "black");
        Metadata.COLOR_MAP.put(G2pBond, "black");
        Metadata.COLOR_MAP.put(G3pBond, "black");
        ...
        Metadata.COLOR_MAP.put(g3Bond, "blue");
        Metadata.COLOR_MAP.put(q3Bond, "blue");
        Metadata.COLOR_MAP.put(kg3Bond,"blue");
        Metadata.DASH_MAP.put(kg3Bond, 3);

        Metadata.COLOR_MAP.put(g4Bond, "orange");
        Metadata.COLOR_MAP.put(q4Bond, "orange");
        Metadata.COLOR_MAP.put(kg4Bond,"orange");
        Metadata.DASH_MAP.put(kg4Bond, 3);
        
        Metadata.COLOR_MAP.put(kg1BondMinus,"yellow");
        Metadata.COLOR_MAP.put(kg2BondMinus,"yellow");
        Metadata.COLOR_MAP.put(kg3BondMinus,"yellow");
        Metadata.COLOR_MAP.put(kg4BondMinus,"yellow");
    ...
    public void makeVirialDiagrams() {
        ...
        if (doShortcut && !flex) {
            ...
            bonds = new SplitParameters(GBond, G1Bond, G2pBond);
            s = split.apply(s, bonds);  
            bonds = new SplitParameters(G1Bond, g1Bond, kg1Bond);
            s = split.apply(s, bonds); 
            bonds = new SplitParameters(G2pBond, G2Bond, G3pBond);
            s = split.apply(s, bonds); 
            bonds = new SplitParameters(G2Bond, g2Bond, kg2Bond);
            s = split.apply(s, bonds);  
            bonds = new SplitParameters(G3pBond, G3Bond, G4Bond);
            s = split.apply(s, bonds);   
            bonds = new SplitParameters(G3Bond, g3Bond, kg3Bond);
            s = split.apply(s, bonds); 
            bonds = new SplitParameters(G4Bond, g4Bond, kg4Bond);
            s = split.apply(s, bonds); 
            ...
            s = split.apply(s, bonds); 
            bonds = new SplitParameters(g3Bond, q3Bond, kg3BondMinus);
            s = split.apply(s, bonds); 
            bonds = new SplitParameters(g4Bond, q4Bond, kg4BondMinus);
            s = split.apply(s, bonds); 
            ...
            for (Graph g : s) {
    			...
    			for (Edge e:g.edges()){// loop over all edges to find kg1BondMinus OR kg2BondMinus OR kg3BondMinus OR kg4BondMinus
    				...
    				if((color == kg1BondMinus) || (color == kg2BondMinus) || (color == kg3BondMinus) || (color == kg4BondMinus)){ 
            ...
            for (Graph g : s) {
    			...
    			for (Edge e:g.edges()){  // loop over all edges in every diagram to find these three types of bonds
    				...
    				if (color == kg3BondMinus){ 
    					e.setColor(kg3Bond); // kg3Minus--> kg3
    				}
    				if (color == kg4BondMinus){ 
    					e.setColor(kg4Bond); // kg4Minus--> kg4
    				}
    			}
    			sTemp.add(g);
            }
            ...
            for (Graph g : s) {
            	...
            	for (Edge e:g.edges()){
    				if ( (e.getColor() == kg1Bond ) || (e.getColor() == q1Bond ) ){
    					...
    					qBond +=2;
    				} else if ( (e.getColor() == kg3Bond ) || (e.getColor() == q3Bond ) ){
    					qBond +=3;
    				} else if ( (e.getColor() == kg4Bond ) || (e.getColor() == q4Bond ) ){
    					qBond +=4;
